...
function roleText(value){
  const row=window.DaGoRoleRankPhraseTable?.[value];
...
function enforceSpecialOrigins(form){
  const roles=new Set(selectedRoles(form));
  form.querySelectorAll('[data-requires-role]').forEach(input=>{
    const ok=roles.has(input.dataset.requiresRole);
    input.disabled=!ok;
    input.closest('label')?.classList.toggle('choice-disabled',!ok);
    if(!ok&&input.checked){
...
function preview(){
  ...
  const text=[
    ...
    `外觀：${selectedRadio(form,'height')}、${selectedRadio(form,'build')}、${selectedRadio(form,'bodyLine')}、${selectedRadio(form,'skinTone')}、${selectedRadio(form,'face')}、${selectedRadio(form,'hairColor')}、${selectedRadio(form,'hairLength')}`,
    `身世：${selectedRadio(form,'specialOrigin')}`,
