...
function randomize(){const form=$('startForm');if(!form)return;if($('playerName'))$('playerName').value=pick(names);['gender','height','build','bodyLine','skinTone','clothes','face','eyeColor','hairColor','hairLength','origin','trait','attributePlan','startSeason','gameMode','difficulty'].forEach(n=>randomizeGroup(form,n));form.querySelectorAll('[name="roles"]').forEach(randomizeSelect);enforceSpecialOrigins(form);const none=form.querySelector('[name="specialOrigin"][value="none"]');if(none)none.checked=true;form.dispatchEvent(new Event('change',{bubbles:true}));preview()}
