(()=>{
'use strict';
const VERSION='1.13.0-bundle-extension';
const DEFAULT_URL='assets/data/dago-changshan-v1-bundle.json?v='+encodeURIComponent(VERSION);
const EXT_URL='assets/data/dago-changshan-v1-extension.json?v='+encodeURIComponent(VERSION);
...
function byId(list,key){const map=new Map();(list||[]).forEach(x=>map.set(x.id||x.passage_code||x.npc_code||x.event_code,x));return map}
function mergeList(base,patch,key){const map=byId(base,key);(patch||[]).forEach(x=>map.set(x.id||x.passage_code||x.npc_code||x.event_code,x));return [...map.values()]}
function mergeBundle(bundle,patch){if(!patch)return bundle;bundle.passages=mergeList(bundle.passages,patch.passages);bundle.relationships=mergeList(bundle.relationships,patch.relationships);bundle.event_pools=mergeList(bundle.event_pools,patch.event_pools);bundle.__bundle_patch=patch.patch_code||'extension';return bundle}
...
async function loadBundle(){
  ...
  for(const attempt of attempts){
    try{const bundle=await attempt.load();if(bundle&&typeof bundle==='object'){try{bundle.__bundle_source=attempt.source;return mergeBundle(bundle,await fetchJson(EXT_URL))}catch{return bundle}}}
...
window.DaGoBundleLoader=Object.freeze({version:VERSION,loadBundle,saveImportedBundle,defaultUrl:DEFAULT_URL,extensionUrl:EXT_URL});
