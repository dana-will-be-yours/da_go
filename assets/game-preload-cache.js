(()=>{
'use strict';
const VERSION='1.9.8-preload-wide-random-fast-sidebars';
...
function makeScrollHtml(lines){return lines.concat(shuffle(lines)).concat(shuffle(lines)).map(t=>'<p class="dago-vtext">'+esc(t)+'</p>').join('')}
const LEFT_LINES=shuffle(SCROLL_LINES);
let RIGHT_LINES=shuffle(SCROLL_LINES);
if(RIGHT_LINES[0]===LEFT_LINES[0])RIGHT_LINES.push(RIGHT_LINES.shift());
const leftScrollHtml=makeScrollHtml(LEFT_LINES);
const rightScrollHtml=makeScrollHtml(RIGHT_LINES);
const css=document.createElement('style');
css.textContent=`#story,#ui-bar{visibility:hidden}body.dago-ready #story,body.dago-ready #ui-bar{visibility:visible}.dago-preload{position:fixed;z-index:9999;inset:0;display:grid;place-items:center;overflow:hidden;isolation:isolate;background:radial-gradient(circle at 50% 35%,rgba(206,213,225,.16),transparent 38%),linear-gradient(135deg,#040506 0%,#0a111b 48%,#020303 100%);color:#eee;font-family:Verdana,Arial,'Microsoft JhengHei',sans-serif}.dago-preload:before{content:'';position:absolute;inset:0;z-index:0;pointer-events:none;background:linear-gradient(90deg,rgba(0,0,0,.6),transparent 26%,transparent 74%,rgba(0,0,0,.65)),linear-gradient(180deg,rgba(255,255,255,.035),rgba(0,0,0,.76))}.dago-preload-card{position:relative;z-index:3;width:min(42rem,92vw);border:1px solid rgba(154,174,204,.34);background:rgba(16,20,29,.9);padding:1.2rem;box-shadow:0 1.2rem 4rem rgba(0,0,0,.58);backdrop-filter:blur(3px)}.dago-preload h1{margin:0 0 .25rem;color:#ffd700;font-size:1.45rem}.dago-preload p{margin:.35rem 0;color:#bbb}.dago-preload-list{display:grid;gap:.35rem;margin:1rem 0}.dago-preload-row{display:flex;justify-content:space-between;gap:1rem;border-bottom:1px solid #444;padding:.35rem 0}.dago-preload-row span{color:#bbb;text-align:right}.dago-preload-row.ok span{color:#8ee88e}.dago-preload-row.bad span{color:#ff8989}.dago-preload-row.warn span{color:#ffd700}.dago-preload-meter{height:.65rem;border:1px solid #555;background:#111;margin:.75rem 0}.dago-preload-meter i{display:block;height:100%;width:0;background:#68a0ff}.dago-preload-actions{display:flex;gap:.5rem;flex-wrap:wrap;margin-top:1rem}.dago-preload button{font:inherit;background:#333;color:#eee;border:1px solid #666;padding:.45rem .8rem;cursor:pointer}.dago-preload button:disabled{opacity:.45}.dago-preload small{color:#888}.dago-preload-atmosphere{position:absolute;inset:0;z-index:1;pointer-events:none}.dago-vertical-log{position:absolute;top:-6vh;width:min(22rem,20vw);height:112vh;overflow:hidden;border-left:1px solid rgba(216,226,240,.24);border-right:1px solid rgba(216,226,240,.2);background:linear-gradient(180deg,rgba(255,255,255,.045),rgba(0,0,0,.34)),rgba(2,5,10,.26);box-shadow:inset 0 0 0 1px rgba(255,255,255,.025),0 0 2.2rem rgba(22,40,62,.22);mask-image:linear-gradient(to bottom,transparent 0%,black 11%,black 89%,transparent 100%);-webkit-mask-image:linear-gradient(to bottom,transparent 0%,black 11%,black 89%,transparent 100%)}.dago-vertical-log.left{left:max(1.5vw,1rem)}.dago-vertical-log.right{right:max(1.5vw,1rem)}.dago-vertical-track{height:max-content;display:flex;flex-direction:row-reverse;align-items:flex-start;justify-content:center;gap:1.25rem;padding:1.8rem 1.05rem;animation:dagoScrollDown 22s linear infinite;will-change:transform}.dago-vertical-log.right .dago-vertical-track{animation-duration:24s;animation-delay:-7s}.dago-vtext{writing-mode:vertical-rl;text-orientation:mixed;margin:0;color:rgba(236,240,247,.78);font-family:'DFKai-SB','標楷體','BiauKai','KaiTi','STKaiti','PMingLiU',serif;font-size:clamp(1.05rem,1.35vw,1.68rem);font-weight:400;line-height:1.66;letter-spacing:.18em;white-space:nowrap;text-shadow:0 0 .55rem rgba(198,216,238,.18)}.dago-vtext:nth-child(3n){color:rgba(210,219,232,.62)}.dago-vtext:nth-child(4n){font-size:clamp(.96rem,1.15vw,1.36rem);opacity:.72}.dago-vtext:nth-child(5n){opacity:.44}@keyframes dagoScrollDown{from{transform:translateY(-52%)}to{transform:translateY(4%)}}@media(max-width:1050px){.dago-vertical-log{width:12rem}.dago-vtext{font-size:1rem}.dago-preload-card{width:min(40rem,76vw)}}@media(max-width:760px){.dago-preload-atmosphere{opacity:.42}.dago-vertical-log{width:7rem}.dago-vertical-log.left{left:.25rem}.dago-vertical-log.right{right:.25rem}.dago-preload-card{background:rgba(16,20,29,.95)}}@media(prefers-reduced-motion:reduce){.dago-vertical-track{animation:none;transform:translateY(-20%)}}`;
document.head.appendChild(css);
const box=document.createElement('section');
box.className='dago-preload';
box.innerHTML='<div class="dago-preload-atmosphere" aria-hidden="true"><div class="dago-vertical-log left"><div class="dago-vertical-track">'+leftScrollHtml+'</div></div><div class="dago-vertical-log right"><div class="dago-vertical-track">'+rightScrollHtml+'</div></div></div><div class="dago-preload-card"><h1>大國年代記</h1><p>正在預載入南京篇快取。必要檔案確認後即可進入遊戲。</p><div class="dago-preload-meter"><i id="dagoPreloadBar"></i></div><div id="dagoPreloadList" class="dago-preload-list"></div><p id="dagoPreloadMsg"><small>準備檢查資源。</small></p><div class="dago-preload-actions"><button id="dagoEnter" type="button" disabled>進入遊戲</button><button id="dagoRetry" type="button">重新讀取快取</button><button id="dagoBypass" type="button">略過快取直接進入</button></div></div>';
