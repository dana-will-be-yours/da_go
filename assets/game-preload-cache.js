(()=>{
'use strict';
const VERSION='1.9.1-preload-cache';
...
function status(a,c,t){const r=rows[a[0]];r.className='dago-preload-row '+c;r.querySelector('span').textContent=t}
async function one(a){status(a,'','讀取中');const res=await fetch(a[0]+'?v='+encodeURIComponent(VERSION),{cache:'no-store'});if(!res.ok)throw new Error('HTTP '+res.status);if(a[3]==='json'){await res.clone().json()}else{const txt=await res.clone().text();if(a[3]&&!txt.includes(a[3]))throw new Error('內容驗證失敗')}if(window.caches){const cache=await caches.open(CACHE_NAME);await cache.put(a[0],res.clone())}}
async function preload(){enter.disabled=true;retry.disabled=true;let done=0,fail=0;bar.style.width='0%';msg.innerHTML='<small>檢查與寫入瀏覽器快取中。</small>';for(const a of ASSETS){try{await one(a);status(a,'ok',a[2]?'已快取':'已快取，可選')}catch(e){status(a,a[2]?'bad':'','失敗：'+String(e.message||e));if(a[2])fail++}done++;bar.style.width=Math.round(done/ASSETS.length*100)+'%'}retry.disabled=false;if(fail){msg.innerHTML='<small>必要資源未通過檢查，請重新讀取快取。</small>';return}msg.innerHTML='<small>快取檢查無誤。可進入遊戲。</small>';enter.disabled=false}
...
async function start(){enter.disabled=true;retry.disabled=true;msg.innerHTML='<small>啟動遊戲主體。</small>';try{await load('assets/game-v6-hotfix.js');await load('assets/game-playable-v6.js');document.body.classList.add('dago-ready');box.remove()}catch(e){msg.innerHTML='<small>啟動失敗：'+String(e.message||e)+'</small>';retry.disabled=false}}
enter.onclick=start;retry.onclick=preload;preload();
})();
