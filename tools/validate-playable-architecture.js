...
must(runtime, "const VERSION='1.14.3-character-build-select-combat';", 'runtime');
for (const token of ['攻擊','自衛','說服','觀察環境','利用環境','使用物品','退避','觀望']) must(select, token, 'DoL-style select combat');
for (const token of ['inner','內功','outer','外功','observe','觀察','speech','口才','body','體魄','tech','技巧','mind','智識']) must(labels, token, 'Chinese skill labels');
for (const token of ['ROLE_EFFECTS','ORIGIN_EFFECTS','TRAIT_EFFECTS','PLAN_EFFECTS','SPECIAL_EFFECTS','specialTalents']) must(build, token, 'complete character creation effects');
...
console.log('Playable architecture validation passed for 1.14.3-character-build-select-combat.');
